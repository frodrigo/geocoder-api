...
docker-compose run --rm --entrypoint /bin/bash addok-fr -c "bzcat addresses/BAN_odbl.json.bz2 | jq -c 'del(.housenumbers[]?.id)' | addok batch"

# Patch BAN
docker-compose run --rm --entrypoint /bin/bash addok-fr -c "ls addresses/*.json | xargs cat | addok batch"

# Index
docker-compose exec addok-fr addok ngrams

# Save redis db into file
docker-compose exec redis-server-fr redis-cli BGSAVE
